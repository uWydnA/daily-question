...
- 如何用Vue点击变色功能的实现
  ...
  <body>
      <div id="box">
          <ul>
              <li :class="index==iNow1?'red':''" v-for='(anyData,index) in arr1' @click='fn(index)'>{{anyData}}</li>
          </ul>
          <input type="text" v-model="mytext">
          <button @click='add()'>add</button>
          <ul>
              <li v-for="(anyData,index) in arr2" v-bind:class="index == iNow?'red':''" @click=addColor(index)>
                  {{anyData}} ---- <button @click='del(index)'>del</button>
              </li>
          </ul>
      </div>
  </body>
...
- 请使用Vue完成模糊搜索
  ...
  ```html
  <div id='box'>
  	<input @input='change' v-model='mytxt'>
      <ul>
          <li v-for='data in arr'></li>
      </ul>
  </div>
  ```
  ...
  let vm = new Vue({
      ...
      data:{
          mytxt:'',
          arr:['aaa','aac','bbb','abc','bbc','ccc','cca','cac'],
          oldarr:['aaa','aac','bbb','abc','bbc','ccc','cca','cac']
      },
      methods:{
          change(){
              this.arr = 	this.oldarr.filter(
                  val=>val.indexOf(this.mytxt) > -1);
          }
      }
  })
  ```
...
- 请使用Vue的计算属性完成模糊搜索
  ...
  let vm = new Vue({
      ...
      data:{
     	...
     	computed:{
          change(){
              return this.arr.filter(val=>val.includes(this.mytxt))
          }
      }
  })
  ```
